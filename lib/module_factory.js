...
function build(factory) {
  ...
  const onComplete = model ? init : onSuccess;
  ...
  for (const m of modules) {
    ...
    if (m.plugin) {
      m.module[PLUGIN] = m.plugin;
    }
    engine.use(m.module, onComplete.bind(this, factory), onError.bind(this, factory));
    count++;
  }
...
function ready(factory) {
  ...
  if (error && callbacks.length <= 0) { throw error; }
