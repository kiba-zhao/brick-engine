...
export class Provider {
  ...
  get pendings() {
    ...
    debug('pendings %s', JSON.stringify(pendings.keys()));
   ...
   */
  contains(id) {
    debug('contains  %s', JSON.stringify(id));
  ...
  async require(...deps) {
    ...
    debug('require %s', JSON.stringify(deps));
  ...
  define(id, deps, factory) {
    ...
    debug('define %s depend on %s', JSON.stringify(id), JSON.stringify(deps));
...
function check(provider, deps, cache = new Map(), ingores = new Map()) {
  ...
  for (const dep of deps) {
    ...
    debug('check with Dependency  %s', JSON.stringify(dep));
...
function buildModel(provider, deps, action) {
  ...
  for (const dep of deps) {
    ...
    debug('buildModel with Dependency  %s', JSON.stringify(dep));
