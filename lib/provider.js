...
export class Provider {
  ...
  constructor(store = new Map()) {
    assert(
      isMap(store),
      '[brick-engine Provider] constructor Error: wrong store'
    );
    this[STORE] = store;
  }
  ...
  async require(...deps) {
    ...
    assert(
      ...
      '[brick-engine Provider] require Error: wrong deps argument'
    );
  ...
  define(id, deps, factory) {
    assert(
      ...
      '[brick-engine Provider] define Error: wrong id argument'
    );
    assert(
      isArray(deps) && deps.every(isProviderDependency),
      '[brick-engine Provider] define Error: wrong deps argument'
    );
    assert(
      isFunction(factory) || isClass(factory),
      '[brick-engine Provider] define Error: wrong factory argument'
    );

    const store = this[STORE];
    assert(
      !store.has(id),
      `[brick-engine Provider] define Error: duplicate ${id.toString()}`
    );
    assert(
      deps.every(_ => _.id !== id),
      `[brick-engine Provider] define Error: circular ${id.toString()}`
    );
...
function check(provider, deps, cache = new Map(), ingores = new Map()) {
  ...
  for (const dep of deps) {
    ...
    assert(
      !ingores.has(id),
      `[brick-engine Provider] check Error:Circular dependency ${id.toString()}`
    );
...
function buildModel(provider, deps, action) {
  ...
  for (const dep of deps) {
    ...
    if (item) {
      ...
      assert(
        required === false,
        `[brick-engine Provider] buildModel Error:dep ${id.toString()} is required`
      );
    }
