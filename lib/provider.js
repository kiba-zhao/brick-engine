...
class Provider {
  constructor(store = {}, opts = DEFAULT_OPTS) {
    ...
    assert(isPlainObject(store), '[brick-engine Provider] constructor Error: wrong store');
    assert(isPlainObject(opts), '[brick-engine Provider] constructor Error: wrong opts');
    assert(isBoolean(opts.cache), '[brick-engine Provider] constructor Error: wrong opts.cache');
  ...
  require(deps, success, fatal) {
    assert(isArray(deps) && deps.every(isDep), '[brick-engine Provider] require Error: wrong deps argument');
    assert(isFunction(success), '[brick-engine Provider] require Error: wrong success argument');
    assert(fatal === undefined || isFunction(fatal), '[brick-engine Provider] require Error: wrong fatal argument');
  ...
  define(id, deps, factory, opts = {}) {
    ...
    assert(isString(id) || isSymbol(id), '[brick-engine Provider] define Error: wrong id argument');
    assert(isArray(deps) && deps.every(isDep), '[brick-engine Provider] define Error: wrong deps argument');
    assert(isPlainObject(opts), '[brick-engine Provider] define Error: wrong opts argument');
    assert((opts.cache === undefined || isBoolean(opts.cache)), '[brick-engine Provider] define Error: wrong opts.cache argument');

    const store = this[STORE];

    assert(store[id] === undefined, `[brick-engine Provider] define Error: store.${id.toString()} exists`);
...
function checkPendings(provider, deps, cache = {}, parent = {}) {
  ...
  for (const dep of deps) {
    ...
    assert(!parent[id], `[brick-engine Provider] checkPendings Error:Circular dependency ${id.toString()}`);
...
function buildModel(provider, deps, action) {
  ...
  for (const dep of deps) {
    ...
    if (item) {
      ...
      assert(required === false, `[brick-engine Provider] buildModel Error:dep ${id.toString()} is required`);
    }
