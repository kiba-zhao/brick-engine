...
export class Provider {
  ...
  constructor(store=new Map()) {
    ...
    assert(isMap(store), '[brick-engine Provider] constructor Error: wrong store');
    this[STORE] = store;
   ...
   */
  async require(...deps) {
    assert(isArray(deps) && deps.every(isProviderDependency), '[brick-engine Provider] require Error: wrong deps argument');
  ...
  define(id, deps, factory) {
    ...
    assert(isProviderStoreKey(id), '[brick-engine Provider] define Error: wrong id argument');
    assert(isArray(deps) && deps.every(isProviderDependency), '[brick-engine Provider] define Error: wrong deps argument');
    assert(isFunction(factory) || isClass(factory), '[brick-engine Provider] define Error: wrong factory argument');

    const store = this[STORE];
    assert(!store.has(id), `[brick-engine Provider] define Error: store.${id.toString()} exists`);
    ...
    if (miss_deps.length > 0) {
      for (const dep of miss_deps) {
        ...
        if (this[PENDINGS].has(dep.id)) {
          dep_pendings = new Map([
            ...this[PENDINGS].get(dep.id),
            ...pendings
          ]);
        }else{
    ...
    for(let pending of pendings.values()) {
...
function isProviderDependency(dep) {
  ...
  return isProviderStoreKey(dep.id) && (isBoolean(dep.required) || dep.required === undefined);
}
 ...
 * @param {Map<ProviderStoreKey,boolean>} parent 父级依赖项字典(用于检查是否存在循环依赖)
...
function check(provider, deps, cache = new Map(), ingores = new Map()) {
  ...
  for (const dep of deps) {
    ...
    assert(!ingores.has(id), `[brick-engine Provider] check Error:Circular dependency ${id.toString()}`);
